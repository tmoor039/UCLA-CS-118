				...
				if(FD_ISSET(fd, &readFds)) {
					...
					if(fd == socket_fd){
						...
						do{
							...
							if(!receive_data(curr_cli_fd, requestMessage)) { break; }
							// Decode the Request
							...
							if(TIMEOUT == 0) { TIMEOUT = string_to_short(header[8]); }
								...
								if (setsockopt(curr_cli_fd, SOL_SOCKET, SO_RCVTIMEO, (char *)&current, sizeof(current)) < 0) {
									close(curr_cli_fd);
									FD_CLR(curr_cli_fd, &watchFds);
								}

								if (setsockopt(curr_cli_fd, SOL_SOCKET, SO_SNDTIMEO, (char *)&current, sizeof(current)) < 0) {
									close(curr_cli_fd);
									FD_CLR(curr_cli_fd, &watchFds);
								}
							}
